(function () {
    ...
    function addDetectionHooks(root, scopeName) {
        ...
        triggers.forEach((e => {
            ...
            console.log("callback", e[detectionType]);
...
function _TTDUniversalPixelApi_1_1_6(optionalTopLevelUrl) {
    ...
    function initializePixel(adv, tag_ids, base_src, dyn_params, uid_config) {
        ...
        async function fireOrDetect(identity_getter, uid_config) {
            try {
                ...
                if (identity) {
                    await firePixelWithUID(identity.advertising_token, UID_IDENTITY_EXISTS);
                }
                ...
                window.__uid2.callbacks.push(async (eventType, payload) => {
                    switch (eventType) {
                        ...
                        case "IdentityUpdated":
                            await firePixelWithUID(payload.identity.advertising_token, UID_IDENTITY_GENERATED)
                            break;
                    }
                });
    ...
    // universal_pixel.js
    this.init = function (adv, tag_ids, base_src, dyn_params, uid_config, route_realtime_api) {
        ...
        if (route_realtime_api) {
            ...
            const realtime_base_src = `https://${rootURL}/track/cei`;
            initializePixel(adv, tag_ids, realtime_base_src, dyn_params, uid_config);
        }
        else {
            initializePixel(adv, tag_ids, base_src, dyn_params, uid_config);
        }
    }
